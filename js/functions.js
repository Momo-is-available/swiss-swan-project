...
// Toggle the social icons display between more and close
function attachToggleMoreListener() {
	const toggleButton = document.getElementById("toggle-more");

	if (toggleButton) {
		toggleButton.addEventListener("click", function () {
			const hiddenItems = document.querySelectorAll(".logo-hidden");

			const isExpanded = this.textContent === "close";

			hiddenItems.forEach((item) => {
				item.classList.toggle("expanded", !isExpanded);
			});

			this.textContent = isExpanded ? "more" : "close";
		});
	} else {
		console.error("Toggle button not found in the footer");
	}
}

/* Shipping_Address form checks */
document.addEventListener("DOMContentLoaded", function () {
	const continueShipping = document.getElementById("continue-shipping");
	if (continueShipping) {
		continueShipping.addEventListener("click", function (event) {
			event.preventDefault();
	...
	if (continuePayment) {
		continuePayment.addEventListener("click", function (event) {
			...
			let transportRadios = document.querySelectorAll(
				"#transport-form input[type='radio']"
			);
			let transportChecked = Array.from(transportRadios).some(
				(radio) => radio.checked
			);

			let paymentRadios = document.querySelectorAll(
				"#payment-method-form input[type='radio']"
			);
			let paymentChecked = Array.from(paymentRadios).some(
				(radio) => radio.checked
			);
